...
### 5.5 對手建模 Opponent Modeling

引擎每次有人宣稱角色時，透過 `notifyClaim()` 通知所有 AI 的 `observe()`，AI 據此累積每位對手的「宣稱歷史」。由於一名玩家當下最多只能真正擁有「影響力張數」種角色，當某對手**宣稱過的角色種類數 > 其手牌數**時即為吹牛嫌疑，`suspicion()` 會據此調高質疑門檻。對手失去影響力（手牌變動）時重置其歷史。實測：對 2 影響力卻宣稱過 3 種角色者，質疑率由 ~0% 升至 ~83%（見 `test/ai-model.js`）。
